...
export function logaUsuario(dados) {
  return(dispatch) => {
    ...
    .post('/login', dados)
    .then(response => {
      api.defaults.headers.common['Authorization'] = response.data.usuario.token // selecionando objetos de outra forma 
...
export function cadastraPostit(dados){
  return (dispatch) => {
    // chamar api
    api
    .post('/postits', dados) // chamando a api com o verbo POST e da onde ela precisa pegar, DADOS
    .then(response => {
